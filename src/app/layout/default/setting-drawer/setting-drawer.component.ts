...
const DEFAULT_VARS = {
  'primary-color': { label: '主颜色', type: 'color', default: '#1c2327' },
...
export class SettingDrawerComponent {
  ...
  changeColor(color: string,colorkey:string) {
  ...
  apply() {
    ...
    //发送颜色
      //判断colorKey是否为undefined，是就把（当前没有点击），上次点击的colorKey传过去
    ...
    this.themeService.eventEmit.emit(this.colorKey)
  }
  ...
  reset() {
    ...
    this.resetData({});
    this.themeService.eventEmit.emit('dark')
  }
